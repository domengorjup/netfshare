...
def available_dirs(mode):
    ...
    all_dirs = [
        dir
        for dir in os.listdir(SHARED_DIRECTORY)
        if os.path.basename(dir) not in app.config["EXCLUDE_DIRNAMES"]
    ]
    matching_mode_paths = [
        dir.path for dir in Directory.query.filter(Directory.mode == mode).all()
    ]
...
def inject_config():
    ...
    context["require_name_id"] = (
        ConfigBool.query.filter(ConfigBool.name == "require_name_id").first().value
    )
    return context
...
def identify():
    ...
    if request.method == "POST":
        ...
        if id:
            ...
            if (
                ConfigBool.query.filter(ConfigBool.name == "require_name_id")
                .first()
                .value
            ):
...
def download(path):
    ...
    if not os.path.isdir(os.path.join(SHARED_DIRECTORY, path)):
        ...
        if os.path.isfile(zip_file):
            if os.path.getmtime(zip_file) < os.path.getmtime(
                os.path.join(SHARED_DIRECTORY, path)
            ):
                refresh_file = True
                print("files changed")
                ...
                for root, dirs, files in os.walk(os.path.join(SHARED_DIRECTORY, path)):
                    for file in files:
                        ...
                        relative_path = os.path.relpath(
                            file_path, os.path.join(SHARED_DIRECTORY, path)
                        )
        ...
        download = Download(
            client_id=client.id,
            directory_id=Directory.query.filter(Directory.path == path).first().id,
        )
...
def upload_dir(path):
    ...
    if request.method == "POST":
        ...
        if ConfigBool.query.filter(ConfigBool.name == "require_name_id").first().value:
            upload_name = (
                client.selected_name.replace(" ", "_") + "_" + client.selected_id
            )
        else:
            upload_name = client.selected_id

        allow_multiple = (
            ConfigBool.query.filter(ConfigBool.name == "allow_multiple_uploads")
            .first()
            .value
        )
        ...
        if len(uploaded_files) > app.config["MAX_FILES"]:
            flash(
                _(
                    "Too many files. Max. %(num_files)d files per upload.",
                    num_files=app.config["MAX_FILES"],
                ),
                "warning",
            )
        ...
        if os.path.exists(target_path):
            if not allow_multiple:
                ...
                flash(
                    _(
                        "An upload with the same ID already exists. Files with matching names were overwritten."
                    ),
                    "warning",
                )
        ...
        flash(
            _(
                "%(num_files)d files successfully uploaded.",
...
def admin_view():
    ...
    manage_dirs = [
        dir for dir in Directory.query.all() if dir.path in os.listdir(SHARED_DIRECTORY)
    ]
    ...
    if check_admin(request):
        ...
        for client in clients:
            try:
                ...
                ping_success = response.success()
            except PermissionError:
                ping_success = False
            if ping_success or client.socket_connected:
                client.last_seen = datetime.datetime.now()
            db.session.commit()

        return render_template(
            "manage_session.html", clients=clients, downloads=downloads, uploads=uploads
        )
    else:
        return redirect(url_for("list_dirs"))
...
def reset_session():
    ...
    if check_admin(request):
        ...
        db.session.commit()
        flash(
            f"Session reset. Deleted {nd_client} client, {nd_download} download and {nd_upload} upload records.",
            "success",
        )
...
def scan_shared_dir():
    ...
    flash(
        f"Shared directory scanned and added {count_added} folders to the database.",
        "success",
    )
    return redirect(url_for("admin_view"))
